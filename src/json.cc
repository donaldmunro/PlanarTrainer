...
//------------------------------------------------------------------------------------------
  ...
  //-----------------------------------------------
   ...
   rapidjson::Value encode_ocv_match(const cv::DMatch& m, rapidjson::Document::AllocatorType& allocator)
   //-----------------------------------------------------------------------------------------------------------
   {
      rapidjson::Value v(rapidjson::kObjectType);
      v.AddMember("trainIdx", rapidjson::Value().SetInt(m.trainIdx), allocator);
      v.AddMember("queryIdx", rapidjson::Value().SetInt(m.queryIdx), allocator);
      v.AddMember("imgIdx", rapidjson::Value().SetInt(m.imgIdx), allocator);
      v.AddMember("distance", rapidjson::Value().SetFloat(m.distance), allocator);
      return v;
   }

   cv::DMatch decode_ocv_match(rapidjson::Value &o)
   //-----------------------------------------------
   {
      cv::DMatch m;
      m.trainIdx = o.FindMember("trainIdx")->value.GetInt();
      m.queryIdx = o.FindMember("queryIdx")->value.GetInt();
      m.imgIdx = o.FindMember("imgIdx")->value.GetInt();
      m.distance = o.FindMember("distance")->value.GetFloat();
      return m;
   }
