#ifndef TRAINER_TYP_H
#define TRAINER_TYP_H
...
struct OGL_params
{
   ...
   int width, height;
   cv::Scalar color;
   std::shared_ptr<std::vector<std::pair<cv::Point2f, cv::Point2f>>> vertices;
   cv::Vec3d normal;
   cv::Vec3d up;
};
...
typedef struct barycentric_info
{
   float area;
   int vertex_keypointno[3];
   float coordinates[3];
} barycentric_info_t;
